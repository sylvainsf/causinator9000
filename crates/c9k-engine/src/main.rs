...
fn format_target(id: &str) -> String {
    if let Some(rest) = id.strip_prefix("job://") {
        ...
        if parts.len() == 4 {
            ...
            return format!(
                "[{run_id}/{job_slug}](https://github.com/{owner}/{repo}/actions/runs/{run_id})"
            );
        }
        if parts.len() == 3 {
...
fn build_json_report(
    ...
    serde_json::json!({
        "schema_version": 1,
        "repo": repo,
        "default_branch": ingest_result.default_branch,
        "hours": hours,
        ...
        "stats": {
            ...
            "user_pr_skipped": ingest_result.skipped_user_prs,
            "trigger_counts": {
...
async fn main() -> Result<()> {
    ...
    if mode == "report" {
        ...
        // Default behaviour: skip contributor PR-branch failures (they are
        // typically `wip` iteration noise on a single contributor's branch
        // and would dominate any repo-wide report). Dependabot PRs are
        // always kept. Pass `--include-user-prs` to opt back in.
        let include_user_prs = args.iter().any(|a| a == "--include-user-prs");
        ...
        // Ingest
        let ingest_result = ingest::ingest_github_with_options(
            &handle,
            &repo,
            hours,
            &ingest::IngestOptions { include_user_prs },
        )?;
        ...
        for (branch, alert_groups) in &branch_buckets {
            ...
            for g in alert_groups {
                ...
                for s in &g.signal_types {
                    all_signals.insert(s.clone());
                }
        ...
        branch_groups.sort_by_key(|g| std::cmp::Reverse(g.total_failures));
        ...
        if !branch_groups.is_empty() || !latent_groups.is_empty() {
            ...
            for bg in &branch_groups {
                ...
                let sha_links: Vec<String> = bg
                    .shas
                    .iter()
                    .map(|sha| format!("[`{sha}`](https://github.com/{repo}/commit/{sha})"))
                    .collect();
        ...
        println!("\n---");
        if ingest_result.skipped_policy > 0 {
            println!(
                "<sub>{} policy/validation workflow failures (checklist, label, CLA, etc.) were excluded from this analysis.</sub>\n",
                ingest_result.skipped_policy
            );
        }
        if ingest_result.skipped_user_prs > 0 {
            println!(
                "<sub>{} user-PR run(s) were excluded (Dependabot PRs are kept). Pass `--include-user-prs` to include them.</sub>\n",
                ingest_result.skipped_user_prs
            );
        }
        println!("*Generated by [Causinator 9000](https://github.com/sylvainsf/causinator9000)*");
        println!();
        println!(
            "<sub>Want help analyzing this report? Copy it and paste it to Copilot with this prompt: \
