...
impl TopologyBuilder {
    ...
    pub fn racks_per_region(mut self, n: usize) -> Self {
        self.racks_per_region = n;
        self
    }
    pub fn vms_per_rack(mut self, n: usize) -> Self {
        self.vms_per_rack = n;
        self
    }
    pub fn containers_per_vm(mut self, n: usize) -> Self {
        self.containers_per_vm = n;
        self
    }
    pub fn identities_per_vm(mut self, n: usize) -> Self {
        self.identities_per_vm = n;
        self
    }
    pub fn apps_per_region(mut self, n: usize) -> Self {
        self.apps_per_region = n;
        self
    }
    pub fn pods_per_app(mut self, n: usize) -> Self {
        self.pods_per_app = n;
        self
    }
    pub fn include_platform_services(mut self, v: bool) -> Self {
        self.include_platform_services = v;
        self
    }
    pub fn include_app_stacks(mut self, v: bool) -> Self {
        self.include_app_stacks = v;
        self
    }
    ...
    pub fn build(self) -> GraphPayload {
        ...
        for r in 0..region_count {
            ...
            if self.include_platform_services {
                ...
                nodes.push(node(
                    &gw_id,
                    &format!("Regional Gateway {region}"),
                    "Gateway",
                    region,
                    None,
                ));
                ...
                nodes.push(node(
                    &idp_id,
                    &format!("IdentityProvider {region}"),
                    "IdentityProvider",
                    region,
                    None,
                ));
                ...
                for mq_idx in 1..=2 {
                    ...
                    nodes.push(node(
                        &mq_id,
                        &format!("MessageQueue {region}/{mq_idx}"),
                        "MessageQueue",
                        region,
                        None,
                    ));
            ...
            for rack in 0..self.racks_per_region {
                ...
                nodes.push(node_with_rack(
                    &tor_id,
                    &format!("ToR {region} rack {rack:03}"),
                    "ToRSwitch",
                    region,
                    &rack_id_str,
                ));
...
mod tests {
    ...
    fn test_minimal_topology() {
        ...
        assert!(
            g.nodes.len() > 20,
            "minimal should have >20 nodes, got {}",
            g.nodes.len()
        );
        assert!(
            g.edges.len() > 15,
            "minimal should have >15 edges, got {}",
            g.edges.len()
        );
    }
    ...
    fn test_standard_topology() {
        ...
        assert!(
            g.nodes.len() > 20000,
            "standard should have >20k nodes, got {}",
            g.nodes.len()
        );
        assert!(
            g.edges.len() > 40000,
            "standard should have >40k edges, got {}",
            g.edges.len()
        );
    }
    ...
    fn test_custom_scale() {
        ...
        assert!(
            g.nodes.len() < 50,
            "small custom should be <50 nodes, got {}",
            g.nodes.len()
        );
    }
    ...
    fn test_large_topology() {
        ...
        assert!(
            g.nodes.len() > 40000,
            "large should have >40k nodes, got {}",
            g.nodes.len()
        );
    }
